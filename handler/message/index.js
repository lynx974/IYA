...
module.exports = msgHandler = async (client = new Client(), message) => {
    try {
        ...
        const devNumber = '628971178515'
        const isDev = sender.id == `${devNumber}@c.us` || false
        ...
        if (isCmd && msgFilter.isFiltered(from) && !isGroupMsg) { return console.log(color('[SPAM]', 'red'), color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname)) }
        if (isCmd && msgFilter.isFiltered(from) && isGroupMsg) { return console.log(color('[SPAM]', 'red'), color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle)) }
        //
        if (!isCmd && !isGroupMsg) { return console.log('[RECV]', color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'Message from', color(pushname)) }
        if (!isCmd && isGroupMsg) { return console.log('[RECV]', color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), 'Message from', color(pushname), 'in', color(name || formattedTitle)) }
        if (isCmd && !isGroupMsg) { console.log(color('[EXEC]'), color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname)) }
        if (isCmd && isGroupMsg) { console.log(color('[EXEC]'), color(moment(t * 25).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`${command} [${args.length}]`), 'from', color(pushname), 'in', color(name || formattedTitle)) }
        ...
        case 'menudev':
            if (!isGroupMsg) return client.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup! [Group Only]', id)
            if (!isGroupAdmins) return client.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup! [Admin Group Only]', id)
            if (!isDev) return client.reply(from, 'Gagal, perintah ini hanya bisa digunakan oleh developer bot!', id)
            await client.sendText(from, menuId.textDev())
            break
        case 'donate':
        case 'donasi':
        ...
        case 'yt':
            if (args.length !== 1) return client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id )
            if (!isUrl(url) & !url.includes('youtube.com') || url.includes('youtu.be')) return client.reply(from, 'Maaf, url yang kamu kirim tidak valid. [Invalid Link]', id)
            await client.reply(from, `_Scraping Metadata.._ \n\n${menuId.textDonasi()}`, id)
            break
        case 'twt':
        case 'twitter':
        ...
        case 'facebook':
            ...
            await client.reply(from, '_Scraping Metadata..._ \n\nTerimakasih telah menggunakan bot ini, ok \nTerimakasih.', id)
        ...
        case 'add':
            ...
            if (groupMembers.includes(`${url}@c.us`)) return await client.reply(from, 'Maaf, user tersebut sudah ada di grup ini.', id)
            await client.addParticipant(groupId, `${url}@c.us`)
            await client.sendTextWithMentions(from, `Request diterima, menambahkan @${url} kedalam grup.`)
            break
        case 'kick':
            if (!isGroupMsg) return client.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup! [Group Only]', id)
            if (!isGroupAdmins) return client.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup! [Admin Group Only]', id)
        ...
        case 'del':
            ...
            break
        case 'tagall':
            if (!isGroupMsg) return client.reply(from, 'Maaf, perintah ini hanya bisa dipakai didalam group saja!.', id)
            if (!isGroupAdmins) return client.reply(from, 'Maaf perintah ini hanya bisa digunakan oleh Admin Groups saja!.', id)
            const groupMem = await client.getGroupMembers(groupId)
            if (args.length === 0) {
                let taglist = `by @${pushname} \n`
                for (let i = 0; i < groupMem.length; i++) {
                    taglist += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
                }
                taglist += '----------------'
                await client.sendTextWithMentions(from, taglist)
            } else {
                let tagListWithMsg = `by @${pushname} \n\n`
                tagListWithMsg += arg + `\n\n`
                for (let i = 0; 1 < groupMem.length; i++) {
                    tagListWithMsg += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
                }
                tagListWithMsg += '----------------'
                await client.sendTextWithMentions(from, tagListWithMsg)
            }
            break
        case 'everyone':
            client.reply(from, 'ehhh, nani nani???', id)
            break
        case 'botstat': {
            if (!isGroupMsg) return await client.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup! [Group Only]', id)
            if (!isGroupAdmins) return await client.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup! [Admin Group Only]', id)
        ...
        case 'say':{
            if (!isGroupMsg) return client.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup! [Group Only]', id)
            const db = ["dih", "lagi ngepart malah dimarahin", "anjing", "dedic anjing", "rep dong kontol", "Yapi Cyna", "halo sayang", "Rela leveling demi lepi meledak", "PART MANA ANJG", "GALIH KONTOL", "KEVIN KERJAIN GFX", "REGGES PELER", "REGGES 10K GW MANA", "be-betsuni >///<", "HENTAI!!", "ettoo:<", "Nani Nani!1!1!", "Apa lu", "APA KONTOL", "ANJING", "LOKONTOL", "ily uwu >///<", "kamu peler", "ANJING LO", "SIAPA LU", "DIH SOK ASIK", "bissmillah hetsod"]
            const randomDB = Math.floor(Math.random() * db.length)
            await client.reply(from, db[randomDB], id)
            break
        }
        case 'apakah':{
            const db = ["Iya", "Gak", "Iya", "Gak", "Iya", "Gak", "Iya", "Gak", "Iya", "Gak"]
            const rolls = Math.floor(Math.random() * db.length)
            await client.reply(from, db[rolls], id)
            break
        }
        // Dev Commands (Dev only)
        case 'kickdev':
            if (!isGroupMsg) return client.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup! [Group Only]', id)
            if (!isDev) return await client.reply(from, 'Gagal, perintah ini hanya bisa digunakan oleh Developer! [Dev Bot Only]', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup! [Bot Not Admin]', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id)
            if (mentionedJidList[0] === botNumber) return await client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id)
            await client.sendTextWithMentions(from, `Request diterima, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
        ...
        break
        case 'join':
            if (!isDev) return client.reply(from, 'Maaf, perintah ini hanya bisa digunakan oleh Developer Bot saja!.', id)
            if (args.length === 1) {
                await client.joinGroupViaLink(url)
            } else if (args.length === 0 && quotedMsg && quotedMsg.type === 'chat') {
                await client.joinGroupViaLink(quotedMsg)
            } else {
